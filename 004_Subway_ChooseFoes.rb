...
# trainer comes from.
#===============================================================================
 def pbSubwayChallengeTrainer(win_count, bttrainers, type)
...
#
#===============================================================================
def pbGenerateSubwayTrainer(idxTrainer, rules)
  bttrainers = pbGetSubwayTrainers(pbSubwayChallenge.currentChallenge)
  btpokemon = pbGetSubwayPokemon(pbSubwayChallenge.currentChallenge)
  level = rules.ruleset.suggestedLevel

  # Verificar se os dados foram carregados corretamente
  raise "Erro: Nenhum treinador encontrado." if bttrainers.nil? || bttrainers.empty?
  raise "Erro: Nenhum Pokémon encontrado." if btpokemon.nil? || btpokemon.empty?
  ...
  # Validar índices de Pokémon
  raise "Erro: Índices de Pokémon inválidos." if pokemonnumbers.nil? || pokemonnumbers.empty?

  if pokemonnumbers.length <= rules.ruleset.suggestedNumber
    pokemonnumbers.each do |n|
      rndpoke = btpokemon[n]
      if rndpoke.nil?
        raise "Erro: Pokémon inválido em índice #{n}"
      end
  ...
  loop do
    ...
    while opponent.party.length < rules.ruleset.suggestedNumber
      ...
      if rndpoke.nil?
        raise "Erro: Pokémon inválido em índice #{rnd}"
      end
      pkmn = rndpoke.createPokemon(level, indvalues, opponent)
      opponent.party.push(pkmn)
    end
